...
that parses a string of email data. The function takes an argument
that contains the data, parses it, then produces two basic
...
The email messages string has the following characteristics:
  The string contains multiple email messages separated by
...
function mailCount(emailData) {
  ...
  const messageParts = messages.map((message) => message.split('#/#'));
  let dates = extractDates(messageParts);
  let [earliestDate, latestDate] = getEarliestAndLatestDate(dates);
...
function extractDates(messageParts) {
  const DATE_ELEMENT = 2;
  /*
  'From: garply@foo.com',
    'Subject: Integer nec nunc facilisis, ultricies.',
    'Date: 07-03-2016', // <-- INDEX 2
    ...
  */
  const START_AFTER_DATE_STRING = 6; // 'Date: 07-27-2016' --> '07-27-2016'

  return messageParts.map(
    (parts) => new Date(parts[DATE_ELEMENT].slice(START_AFTER_DATE_STRING)),
  );
}

function getEarliestAndLatestDate(dates) {
  const dateCopy = [...dates].sort((a, b) => a - b);
  let earliestDate = dateCopy[0];
  let latestDate = dateCopy[dateCopy.length - 1];
